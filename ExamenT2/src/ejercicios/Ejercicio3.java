	...
	public static void main(String[] args) {
		...
		try {
			...
			if (alum != null) {
				...
				if (Dao.existeNotaAsignatura(conn, alum.getDni(), asignaturas.get(elegida))) {
					if (Dao.actualizarNota(conn, alum.getDni(), asignaturas.get(elegida), nota)) {
						System.out.println("Nota actualizada correctamente");
					} else {
						System.out.println("Ha ocurrido un error al actualizar la nota");
					}
				} else {
					if (Dao.insertarNuevaNota(conn, alum.getDni(), asignaturas.get(elegida), nota)) {
						System.out.println("Nota insertada correctamente");
					} else {
						System.out.println("Ha ocurrido un error al insertar la nueva nota");
					}
				}
